...
class Pos extends Component
{
    ...
    public function mount()
    {
        ...
        $needsOrderNumber = !$this->orderNumber && (
            (!$this->orderID && !$this->tableOrderID) ||
            ($this->tableOrderID && $this->tableOrder && !$this->tableOrder->activeOrder)
        );
        if ($needsOrderNumber) {
            $orderNumberData = Order::generateOrderNumber(branch());
    ...
    /** Reset search and menu/category filters from header reset button (browser events pos-reset-filters / pos-reset-search). */
    #[On('pos-reset-filters')]
    #[On('pos-reset-search')]
    public function resetSearchAndFiltersFromHeader()
    {
        $this->search = null;
        $this->menuId = null;
    ...
    {
        // Sync order number from loaded order only (no DB call for new orders - set in mount/resetPos)
        if ($this->orderID && $this->orderDetail && $this->orderDetail->order_number) {
