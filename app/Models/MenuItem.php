...
class MenuItem extends BaseModel
{
    ...
    public function itemPhotoUrl(): Attribute
    {
        return Attribute::get(function (): string {
            ...
            // When using local disk, avoid 404s by returning placeholder if file is missing
            if (config('filesystems.default') === 'local' && !Storage::disk('local')->exists('item/' . $this->image)) {
                return asset('img/food.svg');
            }
            return asset_url_local_s3('item/' . $this->image);
        });
    }
