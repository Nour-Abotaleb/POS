...
const handleShowAddCustomerFromHeader = () => {
    showAddCustomerModal.value = true;
};

const syncHeaderSearchInput = (value) => {
    try {
        const el = document.getElementById("pos-header-search");
        if (!el) return;
        if (document.activeElement === el) return;
        if (el.value !== value) el.value = value;
    } catch (e) {
        // no-op: header might not be present
    }
};

const handleHeaderSetSearch = (event) => {
    const nextValue =
...
const handleHeaderResetSearch = () => {
    handleReset();
};

const handleHeaderResetFilters = () => {
    handleReset();
};
...
const handleReset = () => {
    // Reset reactive state used by MenuPanel
    search.value = "";
    menuId.value = null;
    filterCategories.value = null;

    // Make sure the header search input stays in sync
...
onMounted(async () => {
    if (typeof window !== "undefined") {
        window.addEventListener(
            "pos:show-add-customer",
            handleShowAddCustomerFromHeader,
        );
        window.addEventListener("pos:set-search", handleHeaderSetSearch);
        window.addEventListener("pos:reset-search", handleHeaderResetSearch);
        window.addEventListener("pos:reset-filters", handleHeaderResetFilters);
    }
    ...
    // Ensure header search input is in sync on load
    syncHeaderSearchInput(search.value);
});
...
onUnmounted(() => {
    if (typeof window !== "undefined") {
        ...
        window.removeEventListener("pos:set-search", handleHeaderSetSearch);
        window.removeEventListener("pos:reset-search", handleHeaderResetSearch);
        window.removeEventListener(
            "pos:reset-filters",
            handleHeaderResetFilters,
        );
    }
});

watch(search, (newVal) => {
    syncHeaderSearchInput(newVal || "");
});
