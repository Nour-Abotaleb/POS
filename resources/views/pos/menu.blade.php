...
<div class="w-full flex-1 min-h-0 flex flex-col">
    ...
    <div x-data="{
        ...
        <div class="pos-menu-panel lg:flex flex-col flex-1 min-h-0 gap-3 bg-gray-50 lg:h-full w-full ps-3 dark:bg-gray-900 transition-transform duration-300 md:overflow-hidden" :class="{'hidden': !$store.pos?.showProductsPanel, ' fixed inset-0 top-16 z-[45] flex flex-col': $store.pos?.showProductsPanel}" style="backdrop-filter: blur(2px);" x-cloak>
            {{-- Close button inside panel (mobile/tablet) — in addition to fixed toggle —}}
            <div class="lg:hidden flex-shrink-0 flex justify-end p-3 pb-0">
                <button type="button"
                    @click="$store.pos && ($store.pos.showProductsPanel = false)"
                    class="rounded-full p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-200 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-700 transition"
            ...
            {{-- Menu Items Grid: single scroll container (wrapper); list has no overflow to avoid double scrollbar --}}
            <div
