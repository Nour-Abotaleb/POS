        ...
        <div class="space-y-8">
            @foreach ($tables as $area)
                <div class="flex flex-col gap-3 sm:gap-4 space-y-1" wire:key='area-{{ $area->id . microtime() }}'>
                    <h3 class="f-15 font-medium inline-flex flex-row rtl:flex-row-reverse gap-2 items-center dark:text-neutral-200 ltr:text-left rtl:text-right">{{ $area->area_name }}
                        ...
                        <div class="grid grid-cols-8 gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                            @foreach ($area->tables as $item)
                                ...
                                <div wire:key='table-{{ $item->id . microtime() }}'
                                    ...
                                    @if($isLocked)
                                        <div class="absolute top-1.5 right-1.5 rtl:right-auto rtl:left-1.5 z-10" wire:key="lock-{{ $item->id . '-' . $loop->index . microtime() }}">
                    ...
                    @else
                        <div class="grid sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
                            ...
                            wire:key='table-{{ $item->id . '-' . $loop->index . microtime() }}'
                                href="javascript:;">
                                <div class="flex flex-row rtl:flex-row-reverse items-center gap-4 justify-between w-full cursor-pointer" @if($item->activeOrder) wire:click='showTableOrderDetail({{ $item->id }})' @else wire:click='showTableOrder({{ $item->id }})' @endif>
