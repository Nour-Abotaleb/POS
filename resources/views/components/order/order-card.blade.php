                ...
                <div class="flex flex-col gap-1">
                    ...
                    @if($order->status == 'paid' || $order->status == 'delivered')
                        @if($order->zatca_status == 'reported')
                            <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-green-50 text-green-700 border border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-800 flex items-center gap-1" title="{{ __('settings.zatca_reported') }}">
                        ...
                        @elseif($order->zatca_status == 'failed')
                            <div class="flex flex-col gap-1 items-end">
                                <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-red-50 text-red-700 border border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800 flex items-center gap-1" title="{{ $order->zatca_errors }}">
                                ...
                                <button wire:click.stop="reportToZatca({{ $order->id }})" class="text-[9px] underline text-red-600 hover:text-red-800 dark:text-red-400">
                                    @lang('app.retry')
                                </button>
                            </div>
                        @else
                            <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-gray-50 text-gray-700 border border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 flex items-center gap-1" title="{{ __('settings.zatca_pending') }}">
                                <svg class="w-2.5 h-2.5 animate-pulse" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"></path></svg>
                                ZATCA
                            </span>
                        @endif
                    @endif
