    ...
    event.waitUntil(
        caches.open("app-cache").then((cache) => {
            return fetch("/manifest.json")
                ...
                .then((manifest) => {
                    if (!manifest || !manifest.start_url_base) return Promise.resolve();
                    const fullStartUrl =
                        manifest.start_url_base +
                        (manifest.query_params ? manifest.query_params : "");
                    return cache.add(fullStartUrl).catch(() => {});
                })
                .catch(() => {});
        })
    );
...
self.addEventListener("fetch", (event) => {
    ...
    // Skip user-uploads/item (POS menu images) so 404s fail fast and don't add SW latency
    if (url.pathname.indexOf("/user-uploads/item/") !== -1) {
        return;
    }
