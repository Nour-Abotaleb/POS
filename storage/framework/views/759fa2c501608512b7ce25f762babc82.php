<?php
    // Use the module name key (restapi) for tab resolution
    $tabKey = 'restapi';
    $isActive = ($activeSetting ?? '') === $tabKey;
?>
<li class="me-2">
    <a href="<?php echo e(route('superadmin.superadmin-settings.index') . '?tab=' . $tabKey); ?>" wire:navigate
       class="<?php echo \Illuminate\Support\Arr::toCssClasses([
           'inline-block p-4 border-b-2 rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300',
           'border-transparent' => ! $isActive,
           'active border-skin-base dark:text-skin-base dark:border-skin-base text-skin-base' => $isActive,
       ]); ?>">
        <?php echo e(__('restapi::app.api')); ?>

    </a>
</li>

<?php /**PATH E:\nomu\testfood\POS\Modules/RestApi\Resources/views/sections/superadmin-settings/sidebar.blade.php ENDPATH**/ ?>