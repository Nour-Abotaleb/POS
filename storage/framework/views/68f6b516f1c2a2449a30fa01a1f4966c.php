<style>
    ...
    /* RTL: rotate scroll arrow so it points in the correct direction */
    ...
    /* pe-4 only below md (768px); md and lg+ get no padding-end */
    @media (max-width: 834px) {
        .pos-menu-items-grid {
            padding-inline-end: 1rem !important;
        }
    }
    /* Smooth scrollbar thumb color transition on hover */
    .pos-menu-products-scroll::-webkit-scrollbar-thumb {
...
<div class="w-full h-full min-h-0 flex flex-col">
    ...
    <div x-data="{
        ...
        <!-- Menu Panel: on desktop (lg+) always visible in layout as before. Below lg: hidden until toggle, then fixed overlay below header. -->
        <div class="pos-menu-panel lg:flex flex-col flex-1 min-h-0 gap-3 bg-gray-50 lg:h-full w-full ps-3 dark:bg-gray-900 transition-transform duration-300 md:overflow-hidden" :class="{'hidden': !$store.pos?.showProductsPanel, ' fixed inset-0 top-16 z-[45] flex flex-col': $store.pos?.showProductsPanel}" style="backdrop-filter: blur(2px);" x-cloak>
                        ...
                        <div class="relative space-y-2">
                            ...
                            <template x-if="filterView === 'grid'">
                                <div class="relative flex items-center gap-1 -mx-0.5" x-data="{
                                    ...
                                    get canScrollRight() {
                                        if (this.scrollWidth <= this.clientWidth) return false;
                                        const rtl = document.documentElement.dir === 'rtl';
                                        return rtl ? this.scrollLeft > -this.scrollWidth + this.clientWidth + 2 : this.scrollLeft < this.scrollWidth - this.clientWidth - 2;
                                    },
                                    scroll(dir) {
                                        const el = this.$refs.menuSlider;
                                        if (!el) return;
                                    ...
                                    <div x-ref="menuSlider" class="pos-slider-scroll-hide flex-1 min-w-0 overflow-x-auto overflow-y-hidden pb-2 scroll-smooth">
                                        <div class="flex flex-nowrap items-center gap-2 min-w-0">
                                            <span class="text-sm font-light ms-2 dark:text-gray-300 whitespace-nowrap shrink-0" style="color: #A3A3A399;"><?php echo app('translator')->get('app.posCategory'); ?>:</span>
                                            <button type="button"
                                                wire:click="$set('menuId', null)"
                                                style="<?php echo e($menuId === null ? 'background-color: var(--brand-primary); border-color: var(--brand-primary); color: white;' : ''); ?>"
                                            ...
                                            <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $menuList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $menu): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                ...
                                                <button type="button"
                                                    ...
                                                    style="<?php echo e($isActiveMenu ? 'background-color: var(--brand-primary); border-color: var(--brand-primary); color: white;' : ''); ?>"
                                                    class="<?php echo \Illuminate\Support\Arr::toCssClasses([
                                                        'px-3 py-3 text-xs rounded-lg border transition text-left shrink-0',
                            ...
                            <template x-if="filterView === 'grid'">
                                <div class="relative flex items-center gap-1 -mx-0.5" x-data="{
                                    ...
                                    <div x-ref="productsSlider" class="pos-slider-scroll-hide flex-1 min-w-0 overflow-x-auto overflow-y-hidden pb-2 scroll-smooth">
                                        <div class="flex flex-nowrap items-center gap-2 min-w-0">
                                            <span class="text-sm font-light ms-2 dark:text-gray-300 whitespace-nowrap shrink-0" style="color: #A3A3A399;"><?php echo app('translator')->get('app.posProducts'); ?>:</span>
                                            <button type="button"
                                                wire:click="$set('filterCategories', null)"
                                                style="<?php echo e($filterCategories === null ? 'background-color: var(--brand-primary); border-color: var(--brand-primary); color: white;' : ''); ?>"
                                                class="<?php echo \Illuminate\Support\Arr::toCssClasses([
                                                    'px-3 py-3 text-xs rounded-lg border transition text-left shrink-0',
                                            ...
                                            <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $this->categoryList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                ...
                                                <button type="button"
                                                    ...
                                                    style="<?php echo e($isActiveCategory ? 'background-color: var(--brand-primary); border-color: var(--brand-primary); color: white;' : ''); ?>"
                                                    class="<?php echo \Illuminate\Support\Arr::toCssClasses([
                                                        'px-3 py-3 text-xs rounded-lg border transition text-left shrink-0',
            ...
            <div
                ...
                class="pos-menu-products-scroll flex-1 min-h-0 overflow-y-auto overflow-x-hidden pb-8 scroll-smooth [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:rounded [&::-webkit-scrollbar-track]:bg-gray-200 dark:[&::-webkit-scrollbar-track]:bg-gray-700 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-400 dark:[&::-webkit-scrollbar-thumb]:bg-gray-500 hover:[&::-webkit-scrollbar-thumb]:bg-gray-500 dark:hover:[&::-webkit-scrollbar-thumb]:bg-gray-400 transition-all duration-100"
                @scroll.throttle.100ms="scrollHandler($el)"
                x-data="{
                        ...
                        <li class="group relative flex items-center justify-center">
                            ...
                            <label for="item-<?php echo e($item->id); ?>"
                                class="<?php echo \Illuminate\Support\Arr::toCssClasses([
                                    ...
                                    "cursor-pointer hover:shadow-md dark:hover:bg-gray-700/30 active:scale-95 focus-visible:scale-95 focus-visible:ring-2 focus-visible:ring-skin-base" => !$orderLimitReached && $item->in_stock,
